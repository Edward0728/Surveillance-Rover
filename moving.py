...
#camera = PiCamera()
    ...
    #Initializations of all used pins
def setup():  
...
def motion_detect(channel):
    stop()
    now = datetime.now()
    # retrieve system time for the name of the record video file
...
try: # RF signal detection and corresponding motor control function call
    ...
    while True:
        ...
        if (dist <= 100):# obstacle detection condition
            stop()
            forward_flag = 0 # disable the forward motor function call
            print("Obstacle ahead!")

        if (dist > 100): # clear obstacle detection condition
            forward_flag = 1
